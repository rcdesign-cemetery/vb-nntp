...
function getVbulletinConfig(callback, force) {
  ...
  this._pool.acquire(function (err, db) {
    if (err) {
      callback(err);
      return;
    }
...
function kickBackend(callback) {
  this._pool.acquire(function (err, db) {
    ...
    db.getVbulletinConfig(function (err, config) {
      var request;
...
var Database = module.exports = function Database(options) {
  ...
  this._pool = generic_pool.Pool({
    ...
    create:   function (callback) {
      var db = new mysql.Database(db_options);
      db.connect(function (err, server) {
        this.table = table;
        callback(err, err ? null : db);
      });
    },
    destroy:  function (db) {
      db.disconnect();
...
Database.prototype.getArticle = function (group_id, article_id, callback) {
  ...
  getVbulletinConfig.call(self, function (err, config) {
    ...
    self._pool.acquire(function (err, db) {
      var query;
