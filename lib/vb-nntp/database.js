...
function kickBackend(callback) {
  ...
  this._connect(function (err, db) {
    ...
    getVbulletinConfig.call(self, function (err, config) {
      ...
      self._debug("DATABASE kickBackend() request", request);

      request.on('response', function (response) {
        self._debug("DATABASE kickBackend() response");

        response.on('data', function (chunk) {
          self._debug("DATABASE kickBackend() data", {chunk: chunk});
      ...
      request.on('error', function (err) {
        self._debug('DATABASE kickBackend() error');
        callback(err);
      });
    });
  });
  ...
  this._connect(function (err, db) {
    ...
    query.execute(function (err, rows) {
      ...
      db.query(
        ...
        Object.getOwnProperyNames(s).forEach(function (k) {
          session[k] = s[k];
        });
...
var Database = module.exports = function Database(options, logger) {
  ...
  this._debug = (logger && 'function' === typeof logger.debug)
              ? function () { logger.debug(arguments[0], arguments[1]); }
              : function () {};
