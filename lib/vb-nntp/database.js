  ...
  this._connect(function (err, db) {
    ...
    db.query()
      ...
      .execute(function (err, rows) {
        ...
        self.logger.verbose('DATABASE getVbulletinConfig() got settings');

        rows.forEach(function (row) {
          self.logger.verbose('DATABASE getVbulletinConfig() row', {varname: row.varname, value: row.value});
...
function kickBackend(callback) {
  ...
  this._connect(function (err, db) {
    ...
    getVbulletinConfig.call(self, function (err, config) {
      ...
      self.logger.debug("DATABASE kickBackend() request", request);

      request.on('response', function (response) {
        self.logger.verbose("DATABASE kickBackend() response");

        response.on('data', function (chunk) {
          self.logger.debug("DATABASE kickBackend() data", {chunk: chunk});
