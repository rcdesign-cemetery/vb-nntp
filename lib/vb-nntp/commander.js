...
var CODE = {
  100_HELP_FOLLOWS      : 100,
  111_DATE              : 111,
  201_SRV_READY_RO      : 201,
  211_GRP_SELECTED      : 211,
  215_INFO_FOLLOWS      : 215,
  220_ARTICLE_FOLLOWS   : 220,
  221_HEAD_FOLLOWS      : 221,
  222_BODY_FOLLOWS      : 222,
  224_OVERVIEW_INFO     : 224,
  231_GRP_FOLLOWS       : 231,
  281_AUTH_ACCEPTED     : 281,
  381_AUTH_INCOMPLETE   : 381,
  403_FUCKUP            : 403,
  411_GRP_NOT_FOUND     : 411,
  412_GRP_NOT_SLCTD     : 412,
  420_ARTICLE_NOT_SLCTD : 420,
  423_NO_ARTICLE_IN_GRP : 423,
  480_AUTH_REQUIRED     : 480,
  481_AUTH_REJECTED     : 481,
  482_AUTH_OUT_OF_SEQ   : 482,
  500_CMD_UNKNOWN       : 500,
  501_SYNTAX_ERROR      : 501,
  502_CMD_UNAVAILABLE   : 502
}
...
Commander.prototype.cmdHelp = function (req, res) {
  res.status(CODE.100_HELP_FOLLOWS).write('.').end();
};


Commander.prototype.cmdDate = function (req, res) {
  var now = new Date();
  res.status(CODE.111_DATE, now.getUTCFullYear() +
...
Commander.prototype.cmdAuthInfo = function (req, res) {
  ...
  if (req.session.userid) {
    res.end(CODE.502_CMD_UNAVAILABLE);
    return;
  }

  if ('USER' === req.params[1].toUpperCase()) {
    req.session.username = req.params[2];
    res.end(CODE.381_AUTH_INCOMPLETE);
    return;
  }

  // else arg = PASS
  if (!req.session.username) {
    res.end(CODE.482_AUTH_OUT_OF_SEQ);
    return;
  }
  ...
  self.database.checkAuth(req.session, function (err, verified, bruteforce) {
    ...
    if (verified) {
      ...
      res.end(CODE.281_AUTH_ACCEPTED);
      return;
    }

    if (bruteforce) {
      self.logger.warn('AUTHINFO Brute force deteted', meta);
      res.status(CODE.481_AUTH_REJECTED, 'Authentication rejected (too many attempts)').end();
      return;
    }

    self.logger.notice('AUTHINFO Authentication failed', meta);
    res.end(CODE.481_AUTH_REJECTED);
  });
};
...
Commander.prototype.requiresAuthentication = function (req, res) {
  ...
  res.end(CODE.480_AUTH_REQUIRED);
};
...
Commander.prototype.cmdList = function (req, res) {
  this.database.getGroupsStat(req.session.grp_ids, function (err, rows) {
    ...
    res.status(CODE.215_INFO_FOLLOWS);
...
Commander.prototype.cmdNewGroups = function (req, res) {
  ...
  this.database.getNewGroups(req.session.grp_ids, dt, function (err, rows) {
    ...
    res.status(CODE.231_GRP_FOLLOWS);
    Object.getOwnPropertyNames(req.session.groups).forEach(function (name) {
...
Commander.prototype.cmdGroup = function (req, res) {
  ...
  if (!group_id) {
    res.end(CODE.411_GRP_NOT_FOUND);
    return;
  }
  ...
  this.database.getGroupInfo(group_id, function (err, info) {
    ...
    res.end(CODE.211_GRP_SELECTED, [total, first, last, req.params[1]].join(' '));
  });
};
...
Commander.prototype.cmdListGroup = function (req, res) {
  ...
  this.database.getGroupInfo(group_id, function (err, info) {
    ...
    self.database.getHeaders(group_id, first, last, function(err, hdrs) {
      ...
      res.status(CODE.211_GRP_SELECTED, [total, first, last, group, 'list follows'].join(' '));
...
Commander.prototype.cmdXOver = function (req, res) {
  ...
  if (!req.session.current) {
    res.end(CODE.412_GRP_NOT_SLCTD);
    return;
  }

  if (!req.params[1]) {
    res.end(CODE.420_ARTICLE_NOT_SLCTD);
    return;
  }
  ...
  this.database.getHeaders(group_id, range_min, range_max, function (err, hdrs) {
    ...
    if (!hdrs.length) {
      res.end(CODE.423_NO_ARTICLE_IN_GRP);
      return;
    }

    res.status(CODE.224_OVERVIEW_INFO);
    hdrs.forEach(function (hdr) {
      res.write([
        hdr.messageid,
...
Commander.prototype.cmdXHdr = function (req, res) {
  ...
  this.database.getHeaders(group_id, range_min, range_max, function (err, hdrs) {
    ...
    res.status(CODE.221_HEAD_FOLLOWS);
    hdrs.forEach(function (hdr) {
...
Commander.prototype.cmdArticle = function (req, res) {
  res.status(CODE.220_ARTICLE_FOLLOWS);
...
Commander.prototype.cmdBody = function (req, res) {
  res.status(CODE.222_BODY_FOLLOWS);
  res.write(req.articleBody);
  res.end('.');
};
