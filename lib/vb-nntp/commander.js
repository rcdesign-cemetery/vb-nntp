...
var CODE = {
  _100_HELP_FOLLOWS      : nntp.status(100, 'Help text follows'),
  _111_DATE              : nntp.status(111, null),
  _201_SRV_READY_RO      : nntp.status(201, 'Server ready - No posting allowed'),
  _211_GRP_SELECTED      : nntp.status(211, null),
  _215_INFO_FOLLOWS      : nntp.status(215, 'Information follows'),
  _220_ARTICLE_FOLLOWS   : nntp.status(220, 'Article retrieved - head and body follow'),
  _221_HEAD_FOLLOWS      : nntp.status(221, 'Article retrieved - head follows'),
  _222_BODY_FOLLOWS      : nntp.status(222, 'Article retrieved - body follows'),
  _224_OVERVIEW_INFO     : nntp.status(224, 'Overview information follows'),
  _231_GRP_FOLLOWS       : nntp.status(231, 'List of new newsgroups follows'),
  _281_AUTH_ACCEPTED     : nntp.status(281, 'Authentication accepted'),
  _381_AUTH_INCOMPLETE   : nntp.status(381, 'More authentication information required'),
  _403_FUCKUP            : nntp.status(403, 'Internal fault'),
  _411_GRP_NOT_FOUND     : nntp.status(411, 'No such newsgroup'),
  _412_GRP_NOT_SLCTD     : nntp.status(412, 'No newsgroup has been selected'),
  _420_ARTICLE_NOT_SLCTD : nntp.status(420, 'No current article has been selected'),
  _423_NO_ARTICLE_IN_GRP : nntp.status(423, 'No such article number in this group'),
  _480_AUTH_REQUIRED     : nntp.status(480, 'Authentication required'),
  _481_AUTH_REJECTED     : nntp.status(481, 'Authentication rejected'),
  _482_AUTH_OUT_OF_SEQ   : nntp.status(482, 'Authentication commands issued out of sequence'),
  _500_CMD_UNKNOWN       : nntp.status(500, 'Command not recognized'),
  _501_SYNTAX_ERROR      : nntp.status(501, 'Command syntax error'),
  _502_CMD_UNAVAILABLE   : nntp.status(502, 'Command unavailable')
};

// beautify QUIT response
nntp.status(205, 'Closing connection - Goodbye');
...
function executeHandlersChain(handlers, req, res) {
  ...
  // unref local variables when response is finished
  res.on('end', function () {
    handlers = next = null;
  });
...
function extractRequestMeta(req, err) {
  ...
  if (err) {
    meta.error = err.trace || err.message || err.toString();
  }
...
Commander.prototype.execute = function (req, res) {
  ...
  // unref local variables when response is finished
  res.on('end', function () {
    meta = stack = command = matches = null;
  });
