  ...
  this.database.getHeaders(group_id, range_min, range_max, function (err, hdrs) {
    ...
    hdrs.forEach(function (hdr) {
      res.write([
        ...
        hdr.gmdate,
        msgIdString(this, hdr.postid, hdr.messagetype),
        msgReferers(this, hdr.refid, hdr.messagetype),
        '',
        msgXRef(this, req.session.current, hdr.messageid)
      ].join('\t'));
    });
    res.end('.');
  });
