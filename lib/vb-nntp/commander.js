...
// PRIVATE HELPERS -- request dependant -- fn(req, *args)
////////////////////////////////////////////////////////////////////////////////
...
function msgIdString(req, msgId, msgType) {
  return '<' + msgId + '.' + msgType + '@' + req.session.database.vbconfig.forum_host + '>';
}
...
function msgReferers(req, refererId, msgType) {
  return '<' +refererId + '.' + msgType + '.ref@' + req.session.database.vbconfig.forum_host + '>';
}
...
function msgXRef(req, group, msgId) {
  return 'Xref: ' + req.session.database.vbconfig.forum_host + " " + group + ':' + msgId;
}
 ...
 * Class constructor
 */
var Commander = module.exports = function Commander(logger) {
  this.logger = logger;
 ...
 */
Commander.create = function (logger) {
  return new Commander(logger);
};
...
Commander.prototype.cmdAuthInfo = function (req, res, next) {
  ...
  req.session.database.checkAuth(req.session, function (err, verified, bruteforce) {
    if (err) {
      next(err);
      return;
    }
...
Commander.prototype.cmdList = function (req, res, next) {
  req.session.database.getGroupsStat(req.session.grp_ids, function (err, rows) {
...
Commander.prototype.cmdNewGroups = function (req, res, next) {
  ...
  req.session.database.getNewGroups(req.session.grp_ids, dt, function (err, rows) {
    if (err) {
      next(err);
      return;
    }
...
Commander.prototype.cmdGroup = function (req, res, next) {
  ...
  req.session.database.getGroupInfo(group_id, function (err, info) {
...
Commander.prototype.cmdListGroup = function (req, res, next) {
  ...
  req.session.database.getGroupInfo(group_id, function (err, info) {
    ...
    req.session.database.getHeaders(group_id, first, last, function(err, hdrs) {
      if (err) {
        next(err);
        return;
      }
...
Commander.prototype.cmdXOver = function (req, res, next) {
  ...
  req.session.database.getHeaders(group_id, range_min, range_max, function (err, hdrs) {
    if (err) {
      next(err);
      return;
    }
...
var XHDR_FORMAT = {
  'FROM':       function (hdr) { return [hdr.messageid, msgFrom(hdr.username)]; },
  'SUBJECT':    function (hdr) { return [hdr.messageid, msgSubject(hdr.title)]; },
  'MESSAGE-ID': function (hdr) { return [hdr.messageid, msgIdString(this, hdr.postid, hdr.messagetype)]; },
  'REFERENCES': function (hdr) { return [hdr.messageid, msgReferers(this, hdr.refid, hdr.messagetype)]; },
  'DATE':       function (hdr) { return [hdr.messageid, hdr.gmdate]; },
  '__UNDEF__':  function (hdr) { return [hdr.messageid]; }
...
Commander.prototype.cmdXHdr = function (req, res, next) {
  ...
  range_min = +req.params[2];
  range_max = !!req.params[3] ? (+req.params[4] || req.session.last) : range_min;
  formatter = (XHDR_FORMAT[req.params[1].toUpperCase()] || XHDR_FORMAT.__UNDEF__).bind(req);
...
Commander.prototype.preloadArticle = function (req, res, next) {
  ...
  req.session.database.getArticle(group_id, req.params[1], function (err, article) {
    if (err) {
      next(err);
      return;
    }
